...
async def cmd_help(message: types.Message):
    await message.answer(
        ...
        "Просто отправь фото локомотива или поезда, и я отправлю его на модерацию!"
    )
...
async def handle_photo(message: types.Message):
    ...
    # Отправляем фото в Gemini для анализа
    try:
        # Скачиваем фото
...
1. Что изображено (тип локомотива/поезда, если видно)
2. Качество фотографии (резкость, освещение, композиция)
3. Подходит ли для публикации в фотогалерее

Дай краткий анализ на русском языке (2-3 предложения)."""
    ...
    admin_ids = ADMIN_CHAT_ID.strip('[]').split(',')
...
# ==================== WEBHOOK ДЛЯ VERCEL ====================
app = FastAPI()
WEBHOOK_URL = "https://skzd-photo-bot.vercel.app/webhook"
...
@app.get("/")
async def root():
    return {"message": "SKZD Photo Bot is running! 🚂"}
